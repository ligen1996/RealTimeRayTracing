...
def render_graph_PathTracerGraph():
    g = RenderGraph('CompareRTAccumulate')
    ...
    SplitScreenPass = createPass('SplitScreenPass', {'splitLocation': 0.5076388716697693, 'showTextLabels': False, 'leftLabel': 'Left side', 'rightLabel': 'Right side'})
    ...
    AccumulatePass_ = createPass('AccumulatePass', {'enabled': True, 'autoReset': True, 'precisionMode': AccumulatePrecision.Single, 'subFrameCount': 0, 'maxAccumulatedFrames': 100})
    g.addPass(AccumulatePass_, 'AccumulatePass_')
    GaussianBlur = createPass('GaussianBlur', {'kernelWidth': 5, 'sigma': 2.0})
    g.addPass(GaussianBlur, 'GaussianBlur')
    GaussianBlur_ = createPass('GaussianBlur', {'kernelWidth': 5, 'sigma': 2.0})
    g.addPass(GaussianBlur_, 'GaussianBlur_')
    ...
    g.addEdge('SpatioTemporalSM.Visibility', 'AccumulatePass_.input')
    g.addEdge('AccumulatePass_.output', 'GaussianBlur.src')
    g.addEdge('GaussianBlur.dst', 'SplitScreenPass.rightInput')
    g.addEdge('ToneMappingPass.dst', 'GaussianBlur_.src')
    g.addEdge('GaussianBlur_.dst', 'SplitScreenPass.leftInput')
